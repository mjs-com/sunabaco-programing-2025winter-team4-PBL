...
Vercelにデプロイする前に、ローカルでエラーチェックを行うことで、デプロイ失敗を防げます。

### TypeScript型チェック（推奨）

Vercelで最も多いエラーは**TypeScript型エラー**です。以下のコマンドで事前にチェックできます：

```powershell
npx tsc --noEmit
```

- エラーがなければ何も表示されません（正常）
- エラーがあれば、該当ファイルと行番号が表示されます
...
本番環境と同じビルドを実行してエラーを確認：

```powershell
npm run build
```

**注意**: ビルドには数分かかる場合があります。急いでいる場合は `npx tsc --noEmit` だけでも十分です。

### チェック手順のまとめ

デプロイ前に以下の順番でチェックすることを推奨します：

1. `npx tsc --noEmit` - 型エラーの確認（必須）
2. `npm run lint` - コード品質の確認（推奨）
3. `npm run build` - 完全ビルドテスト（念のため）
